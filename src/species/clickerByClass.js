const getDefaultConfig = (randomizer, window) => {
    ...
    const defaultShowAction = (x, y) => {
        ...
        clickSignal.style.border = '3px solid cyan';
    ...
    return {
        ...
        log: false,
        className: null, // Requires user configuration
    };
};
...
export default (userConfig) => ({ logger, randomizer, window }) => {
    ...
    if (!config.className) {
        if (logger && config.log) {
            logger.log('gremlin', 'clickerByClass', 'skipped', 'className config not provided');
        }
        return () => {}; // Return a no-op function if not configured
    }

    return () => {
        const elements = [
            ...document.querySelectorAll(`.${config.className}`),
        ];

        if (elements.length === 0) return;

        const targetElement = randomizer.pick(elements);
        ...
        if (logger && config.log) {
            logger.log('gremlin', 'clickerByClass', 'click', `.${config.className}`, 'at', posX, posY);
        }
    };
};
