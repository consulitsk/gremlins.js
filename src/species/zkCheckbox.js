const getDefaultConfig = (randomizer, window) => {
    ...
    const defaultCanClick = (element) => {
        return (
            ...
            element.classList.contains('z-checkbox')
        );
    };
    ...
    const defaultShowAction = (x, y) => {
        ...
        clickSignal.style.border = '3px solid green';
...
export default (userConfig) => ({ logger, randomizer, window }) => {
    ...
    return () => {
        const checkboxes = [
            ...document.querySelectorAll('.z-checkbox'),
        ];
        if (checkboxes.length === 0) return;

        const targetElement = randomizer.pick(checkboxes);
        ...
        if (logger && config.log) {
            logger.log('gremlin', 'zkCheckbox', 'click', 'at', posX, posY);
        }
    };
};
